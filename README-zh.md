...
- **如何设计 Agent 可执行的守护条件** — Comet 的阶段退出不是简单相信 Agent 说“完成了”，而是通过 `comet-guard.js`、`comet-yaml-validate.js`、`comet-state.js` 等 Node 脚本检查任务、状态字段、验证证据和归档条件，满足条件后才允许推进。
...
| 脚本 | 用途 |
|--------|---------|
| `comet-env.js` | 脚本发现助手 — 以 JSON 输出 `COMET_GUARD`、`COMET_STATE`、`COMET_HANDOFF`、`COMET_ARCHIVE` 等内置脚本路径 |
| `comet-guard.js` | 阶段转换守护 — 验证退出条件，`--apply` 自动更新 `.comet.yaml` |
| `comet-handoff.js` | 设计交接 — 从 OpenSpec 制品生成带 SHA256 追踪的确定性上下文包 |
| `comet-archive.js` | 一键归档 — 验证状态、同步 specs、移至归档、更新状态 |
| `comet-yaml-validate.js` | 模式校验器 — 校验 `.comet.yaml` 结构和字段值 |
| `comet-state.js` | 统一状态管理 — init/set/get/check/scale，agent 的专属 YAML 接口 |
...
2. **自动化状态转换** — `comet-guard.js --apply` 自动更新 `.comet.yaml`
   - 所有阶段转换（open → design/build → verify → archive）使用 `guard --apply`
   - 无需手动状态编辑 — 消除写入验证错误
   - `comet-state.js` 是 agent 对状态操作的专属接口
   - Guard 和 archive 脚本内部使用 `comet-state.js` 进行状态管理

3. **模式校验** — `comet-yaml-validate.js` 确保数据完整性
   - 校验必填字段和可选字段
...
6. **归档自动化** — `comet-archive.js` 一键处理完整归档流程
...
│   ├── comet/SKILL.md
│   │   └── scripts/
│   │       ├── comet-guard.js       # 阶段转换守护（--apply 自动更新状态）
│   │       ├── comet-env.js         # 脚本发现助手
│   │       ├── comet-handoff.js     # 设计交接（OpenSpec → Superpowers 上下文追踪）
│   │       ├── comet-archive.js     # 一键归档自动化
│   │       ├── comet-yaml-validate.js # 模式校验器
│   │       └── comet-state.js       # 统一状态管理（init/set/get/check/scale）
│   ├── comet-*/SKILL.md
│   ├── openspec-*/SKILL.md
