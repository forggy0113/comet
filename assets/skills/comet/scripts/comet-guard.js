...
import { fieldValue, fileNonempty, green, hashFile, red, runCommand, runCommandString, runNode, scriptDir, sha256, slash, validateChangeName, yellow, } from './comet-lib.js';
=======
...
function tasksHasAny() {
    return (existsSync(join(changeDir, 'tasks.md')) &&
        /- \[/.test(readFileSync(join(changeDir, 'tasks.md'), 'utf8')));
}
function projectConfigValue(field) {
    const local = y(field);
    if (local && local !== 'null')
        return local;
...
function buildPasses() {
    ...
    if (configured) {
        ...
        if (result.status !== 0)
            throw new Error(`command failed: ${configured}`);
        return;
    }
...
function verificationCommandPasses() {
    if (process.env.COMET_SKIP_BUILD === '1')
        return;
...
function designHandoffContextValid() {
...
function designHandoffMarkdownTraceable() {
    const context = y('handoff_context');
    if (!context || context === 'null')
        throw new Error('handoff_context is missing from .comet.yaml');
    const markdown = context.replace(/\.json$/, '.md');
...
function frontmatterHas(file, field, expected) {
...
function guardDesign() {
    ...
    if (designDoc && designDoc !== 'null') {
        ...
        check('Design Doc frontmatter links current change', () => frontmatterHas(designDoc, 'comet_change', change));
        check('Design Doc declares technical design role', () => frontmatterHas(designDoc, 'role', 'technical-design'));
        check('Design Doc declares OpenSpec as canonical spec', () => frontmatterHas(designDoc, 'canonical_spec', 'openspec'));
    }
    else
        yellow('  [WARN] No design_doc recorded in .comet.yaml');
}
function guardBuild() {
    console.error('=== Guard: build → verify ===');
    check('isolation selected', () => ['branch', 'worktree'].includes(y('isolation')) ||
    ...
    check('build_mode selected', () => ['subagent-driven-development', 'executing-plans', 'direct'].includes(y('build_mode')) ||
    ...
    check('build_mode allowed for workflow', () => y('build_mode') !== 'direct' ||
...
if (block) {
    ...
    process.exit(1);
=======
if (phase === 'archive' && !existsSync(changeDir) && existsSync(join('openspec', 'changes', 'archive', change))) {
  changeDir = join('openspec', 'changes', 'archive', change);
}
const yaml = join(changeDir, '.comet.yaml');
let block = false;
...
function tasksAllDone() {
  const tasks = join(changeDir, 'tasks.md');
  if (!existsSync(tasks)) throw new Error(`tasks.md is missing at ${tasks}\nNext: restore or create tasks.md for this change before leaving build.`);
  const text = readFileSync(tasks, 'utf8');
  if (!/- \[x\]/.test(text)) throw new Error("tasks.md has no completed tasks.\nNext: complete implementation tasks and mark them with '- [x]'.");
  const unfinished = text.split(/\r?\n/).map((line, index) => ({ line, index: index + 1 })).filter(({ line }) => /- \[ \]/.test(line));
  if (unfinished.length > 0) {
    throw new Error(`Unfinished tasks:\n${unfinished.map(({ line, index }) => `${index}:${line}`).join('\n')}\nNext: complete or explicitly remove unfinished tasks, then mark tasks.md with '- [x]'.`);
  }
}
...
function tasksHasAny() {
  return existsSync(join(changeDir, 'tasks.md')) && /- \[/.test(readFileSync(join(changeDir, 'tasks.md'), 'utf8'));
}
...
function buildPasses() {
  ...
  if (existsSync('pom.xml')) {
    const result = existsSync('./mvnw') ? runCommand(['./mvnw', 'compile', '-q']) : runCommand(['mvn', 'compile', '-q']);
...
function sourceFiles() {
  const files = [join(changeDir, 'proposal.md'), join(changeDir, 'design.md'), join(changeDir, 'tasks.md')];
  const specsRoot = join(changeDir, 'specs');
  if (existsSync(specsRoot)) {
...
function computeHandoffHash() {
  let content = '';
  for (const file of sourceFiles()) if (existsSync(file)) content += `path:${slash(file)}\nsha256:${hashFile(file)}\n`;
  return sha256(content);
}
...
function designHandoffContextValid() {
  const context = y('handoff_context');
  const recordedHash = y('handoff_hash');
  if (!context || context === 'null') throw new Error('handoff_context is missing from .comet.yaml\nNext: run node "$COMET_HANDOFF" <change> design --write before invoking Superpowers.');
  if (!fileNonempty(context)) throw new Error(`handoff_context does not point to a non-empty file: ${context}\nNext: regenerate the design handoff with comet-handoff.js.`);
  if (!/^[a-f0-9]{64}$/.test(recordedHash)) throw new Error(`handoff_hash is missing or invalid: ${recordedHash || 'null'}\nNext: regenerate the design handoff with comet-handoff.js.`);
  const actualHash = computeHandoffHash();
  if (actualHash !== recordedHash) throw new Error(`OpenSpec artifacts changed after handoff was generated.\nExpected handoff_hash: ${recordedHash}\nActual handoff_hash:   ${actualHash}\nNext: rerun comet-handoff.js so Superpowers receives the current OpenSpec context.`);
  const markdown = context.replace(/\.json$/, '.md');
  if (!fileNonempty(markdown)) throw new Error(`design handoff markdown is missing or empty: ${markdown}\nNext: regenerate the design handoff with comet-handoff.js.`);
}
...
function designHandoffMarkdownTraceable() {
  ...
  const text = readFileSync(markdown, 'utf8');
  const missing = [];
  if (!/^Generated-by: comet-handoff\.js$/m.test(text)) missing.push('handoff markdown is missing Generated-by marker');
  if (!/^- Mode: (compact|full)$/m.test(text)) missing.push('handoff markdown is missing Mode marker');
  for (const file of sourceFiles()) {
    if (!existsSync(file)) continue;
    if (!text.includes(`- Source: ${slash(file)}`)) missing.push(`handoff markdown is missing source reference: ${slash(file)}`);
    if (!text.includes(`- SHA256: ${hashFile(file)}`)) missing.push(`handoff markdown is missing current sha256 for: ${file}`);
  }
...
function guardBuild() {
  console.error('=== Guard: build → verify ===');
  check('isolation selected', () => ['branch', 'worktree'].includes(y('isolation')) || `isolation must be branch or worktree, got '${y('isolation') || 'null'}'\nNext: ask the user to choose branch or worktree, create the chosen isolation, then run:\n  node "$COMET_STATE" set ${change} isolation <branch|worktree>`);
  check('build_mode selected', () => ['subagent-driven-development', 'executing-plans', 'direct'].includes(y('build_mode')) || `build_mode must be selected before leaving build, got '${y('build_mode') || 'null'}'\nNext: ask the user to choose an execution mode, then run:\n  node "$COMET_STATE" set ${change} build_mode <subagent-driven-development|executing-plans>`);
  check('build_mode allowed for workflow', () => y('build_mode') !== 'direct' || ['hotfix', 'tweak'].includes(y('workflow')) || y('direct_override') === 'true' || 'build_mode=direct is only allowed for hotfix/tweak unless direct_override: true is recorded\nNext: choose executing-plans or subagent-driven-development, or stop and ask the user for an explicit direct override.');
  check('tasks.md all tasks checked', tasksAllDone);
  check('proposal.md exists', () => fileNonempty(join(changeDir, 'proposal.md')));
  check('Build passes', buildPasses);
}
...
function guardVerify() {
  ...
  check('tasks.md all tasks checked', tasksAllDone);
  check('Build passes', verificationCommandPasses);
  check('verification_report exists', () => y('verification_report') && y('verification_report') !== 'null' && existsSync(y('verification_report')));
  check('branch_status=handled', () => y('branch_status') === 'handled');
}
...
function applyStateUpdate() {
  ...
  const event = { open: 'open-complete', design: 'design-complete', build: 'build-complete', verify: 'verify-pass' }[phase];
  if (event) {
    const result = runNode(state, ['transition', change, event]);
    if (result.status !== 0) {
      process.stderr.write(result.stderr || '');
...
preflight();
({ open: guardOpen, design: guardDesign, build: guardBuild, verify: guardVerify, archive: guardArchive }[phase])();
if (block) {
  console.error('');
  red('BLOCKED — fix failing checks before proceeding to next phase');
  process.exit(1);
