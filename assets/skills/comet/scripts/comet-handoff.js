...
function sourceFiles(changeDir) {
    return [
        ...
        ...walkSpecFiles(join(changeDir, 'specs')),
    ];
}
...
function writeMarkdown(output, files, change, mode, contextHash) {
    ...
    for (const file of files) {
        ...
        if (mode === 'full' || lines.length <= 80)
            chunks.push('```md', text.replace(/\s*$/, ''), '```');
        else
            chunks.push('[TRUNCATED]', '', '```md', lines.slice(0, 80).join('\n'), '```', '', `Full source: ${slash(file)}`);
        chunks.push('');
    }
...
function writeJson(output, files, change, mode, contextHash) {
    const body = {
        ...
        files: files
            ...
            .map((file) => ({ path: slash(file), sha256: hashFile(file) })),
    };
    writeFileSync(output, `${JSON.stringify(body, null, 2)}\n`);
}
const [change, phase, modeArg, fullFlag] = process.argv.slice(2);
validateChangeName(change);
if (phase !== 'design' || modeArg !== '--write' || (fullFlag && fullFlag !== '--full'))
    error('Usage: comet-handoff.js <change-name> design --write [--full]');
...
if (!existsSync(changeDir))
    error(`ERROR: change directory not found: ${changeDir}`);
if (!existsSync(yaml))
    error(`ERROR: .comet.yaml not found at ${yaml}`);
if (fieldValue(yaml, 'phase') !== 'design')
    error('ERROR: design handoff requires phase: design');
for (const required of ['proposal.md', 'design.md', 'tasks.md']) {
    if (!fileNonempty(join(changeDir, required)))
        error(`ERROR: required OpenSpec artifact missing or empty: ${join(changeDir, required)}`);
}
...
if (!existsSync(stateScript))
    error('ERROR: comet-state.js not found; cannot record handoff fields');
