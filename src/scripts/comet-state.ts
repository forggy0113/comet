...
function get(change: string, field: string): void {
...
function getValue(change: string, field: string): string {
  const yaml = yamlFileFor(change);
  return fieldValue(yaml, field);
}

function set(change: string, field: string, value: string, quiet: boolean = false): void {
  validateChangeName(change);
  const yaml = yamlFileFor(change);
  if (!existsSync(yaml)) fail(`.comet.yaml not found at ${yaml}`);
...
function init(change: string, workflow: string): void {
  validateChangeName(change);
...
function requirePhase(change: string, expected: string): void {
  const actual = getValue(change, 'phase');
  if (actual !== expected)
    fail(`Cannot transition '${change}': expected phase ${expected}, got ${actual}`);
}

function requireVerificationEvidence(change: string): void {
  ...
  if (!report || report === 'null' || !existsSync(report)) {
    fail(
      `Cannot transition '${change}': verification_report must point to an existing report file`,
    );
  }
...
function requireBuildDecisions(change: string): void {
  ...
  if (!['branch', 'worktree'].includes(isolation)) {
    fail(
      `Cannot transition '${change}': isolation must be branch or worktree, got '${isolation || 'null'}'`,
    );
  }
  if (!ENUMS.build_mode.includes(buildMode)) {
    fail(
      `Cannot transition '${change}': build_mode must be selected before leaving build, got '${buildMode || 'null'}'`,
    );
  }
  if (
    buildMode === 'direct' &&
    ...
    fail(
      `Cannot transition '${change}': build_mode=direct is only allowed for hotfix/tweak unless direct_override=true`,
    );
  }
}

function transition(change: string, event: string): void {
  validateChangeName(change);
  validateEnum(event, [
    'open-complete',
    'design-complete',
...
function checkNonempty(desc: string, file: string): void {
  if (fileNonempty(file)) pass(`${desc} non-empty`);
  else block(`${desc} missing or empty`);
}
function checkYamlIs(field: string, expected: string, change: string): void {
  const actual = getValue(change, field);
  if (actual === expected) pass(`${field}=${actual} (expected: ${expected})`);
  else block(`${field}=${actual} (expected: ${expected})`);
}
function checkYamlEmpty(field: string, change: string): void {
  const value = getValue(change, field);
  if (!value || value === 'null') pass(`${field} is empty/null`);
  else block(`${field}=${value} (expected: empty/null)`);
}

function check(change: string, phase: string): void {
  validateChangeName(change);
  ...
  if (phase === 'open') {
    ...
    if (workflow === 'full') {
      ...
      else block(`design_doc=${designDoc} (expected: non-null and file exists)`);
    } else {
    ...
    else block(`verify_result=${result} (expected: pending or null)`);
    ...
    if (archived !== 'true') pass(`archived=${archived} (expected: not true)`);
    else block(`archived=${archived} (expected: not true)`);
  }
  console.log('');
  if (checkBlock) {
...
function fieldStatus(field: string, value: string, file: string = ''): void {
...
function recover(change: string): void {
  validateChangeName(change);
  const changeDir = join('openspec', 'changes', change);
  const yaml = join(changeDir, '.comet.yaml');
  if (!existsSync(yaml)) fail(`.comet.yaml not found at ${yaml}`);
  ...
  if (phase === 'open') {
    ...
    for (const f of ['proposal.md', 'design.md', 'tasks.md'])
      console.log(
        `  - ${f}: ${fileNonempty(join(changeDir, f)) ? 'DONE' : 'MISSING (unexpected in design phase)'}`,
      );
    console.log('\n  Design progress:');
    ...
    if (
      ...
      console.log(
        'Recovery action: Isolation not selected. Use AskUserQuestion to ask user for branch/worktree choice.',
      );
    else if (!values.build_mode || values.build_mode === 'null')
      console.log(
        'Recovery action: Build mode not selected. Use AskUserQuestion to ask user for execution method.',
      );
    ...
    if (values.verify_result === 'pass' && values.branch_status === 'handled')
      ...
      console.log(
        'Recovery action: Verification passed but branch not yet handled. Complete branch handling and set branch_status to handled.',
      );
...
function scale(change: string): void {
  validateChangeName(change);
  const changeDir = join('openspec', 'changes', change);
  const yaml = join(changeDir, '.comet.yaml');
  if (!existsSync(yaml)) fail(`.comet.yaml not found at ${yaml}`);
  ...
  if (git(['rev-parse', '--git-dir']).status === 0) {
    ...
    const diff =
      baseRef && baseRef !== 'null' && git(['rev-parse', '--verify', baseRef]).status === 0
...
if (subcommand === 'init' && args.length >= 2) init(args[0], args[1]);
  ...
  if (args[2] === '--recover') recover(args[0]);
  else check(args[0], args[1]);
